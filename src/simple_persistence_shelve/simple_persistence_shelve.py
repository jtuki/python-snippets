...
def test_shelve_insert_new(n):
    ...
    db = shelve.open(db_file, 'n')      # no writeback option enabled
...
db_writeback_index_name = "map_db"
    
def test_shelve_insert_new_writeback(n):
    global db_file
    global gl_keywords_list
    ...
    db[db_writeback_index_name] = dict()
    for w in gl_keywords_list:
        db[db_writeback_index_name][w] = w*3
    db.close() # automatically sync and close

def test_shelve_random_update_writeback(n):
    global db_file
    global words
    global gl_keywords_list
    global gl_unordered_list
    ...
    for i in gl_unordered_list:
        w = gl_keywords_list[i]
        db[db_writeback_index_name][w] = w*2
    db.close()

def test_shelve_random_read_writeback(n):
    global db_file
    global words
    global gl_keywords_list
    assert len(gl_keywords_list) == n
    ...
    for i in gl_unordered_list:
        w = gl_keywords_list[i]
        r = db[db_writeback_index_name][w]
    db.close()
    
def test_shelve_random_read_writeback_faster(n):
    global db_file
    global words
    global gl_keywords_list
    assert len(gl_keywords_list) == n
    ...
    db = shelve.open(db_file, 'r')
    m = db[db_writeback_index_name]
    for i in gl_unordered_list:
        w = gl_keywords_list[i]
        r = m[w]
    db.close()
    
def test_shelve_random_delete_writeback(n):
    global db_file
    global words
    global gl_keywords_list
    assert len(gl_keywords_list) == n
    ...
    for i in gl_unordered_list:
        w = gl_keywords_list[i]
        del db[db_writeback_index_name][w]
    db.close()
...
if __name__ == '__main__':
    ...
    print ("## writeback not used => only assignment or del")
    ...
    print ("## writeback used => list append or new map member assignment etc.")
    
    t = timeit.timeit("test_shelve_insert_new_writeback(%d)" % (n_keywords), "from __main__ import test_shelve_insert_new_writeback", number=1)
    print("test_shelve_insert_new_writeback(%d): %f seconds" % (n_keywords, t))
    
    t = timeit.timeit("test_shelve_random_update_writeback(%d)" % (n_keywords), "from __main__ import test_shelve_random_update_writeback", number=1)
    print("test_shelve_random_update_writeback(%d): %f seconds" % (n_keywords, t))
    
    t = timeit.timeit("test_shelve_random_read_writeback(%d)" % (n_keywords), "from __main__ import test_shelve_random_read_writeback", number=1)
    print("test_shelve_random_read_writeback(%d): %f seconds" % (n_keywords, t))
    
    t = timeit.timeit("test_shelve_random_read_writeback_faster(%d)" % (n_keywords), "from __main__ import test_shelve_random_read_writeback_faster", number=1)
    print("test_shelve_random_read_writeback_faster(%d): %f seconds" % (n_keywords, t))
    
    t = timeit.timeit("test_shelve_random_delete_writeback(%d)" % (n_keywords), "from __main__ import test_shelve_random_delete_writeback", number=1)
    print("test_shelve_random_delete_writeback(%d): %f seconds" % (n_keywords, t))
